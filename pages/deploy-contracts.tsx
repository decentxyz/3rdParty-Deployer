...
const schema = yup.object().shape({
  ...
  symbol: yup.string().typeError("Give your collection a symbol."),
  tokenPrice: yup
    .number()
    .typeError(
      "Must set price for token. Please set to 0 if you wish for your NFTs to be free."
    ),
  saleStartDate: yup.number().typeError("Must set."),
  saleEndDate: yup.number().typeError("Must set."),
  editionSize: yup
    .number()
  ...
  revPathAddress: yup.string().required("Set Revenue Path Address."),
...
const Deploy: NextPage = () => {
  ...
  const deployFunction = async () => {
    ...
    try {
      if (!signer) {
        ...
        const ipfsAudio =
          (audioFile as any)?.raw?.size && (audioFile as any)?.raw?.size > 0
            ? await promise(audioFile.raw)
            : ipfsImg;
        ...
        try {
          nft = await edition.deploy(
            ...
            (pending: any) => {
              console.log("Pending nonce: ", pending.nonce);
            },
            (receipt: any) => {
  ...
  useEffect(() => {
    if (typeof window !== "undefined")
      (window as any).meta = (address: string) => {
  ...
  return (
    <>
      <div className="flex flex-col gap-8 items-center justify-center min-h-screen text-white pt-28 pb-16 px-16">
        <h1 className="text-white border-b border-white w-full py-2 mb-8">
          Start Decent x Reveel
        </h1>
            ...
            <div className="flex gap-20">
              <div className="space-y-8  w-[500px]">
                <div className="flex flex-col gap-3 border border-white rounded-md p-4">
                  <p className="text-lg font-medium">
                    1. Add split recipients for this project & deploy your
                    revenue path.
                  </p>
                  <p className="flex justify-between font-header">
                    Reveel payout address
                  </p>
                  <input
                    placeholder="0x000"
                    disabled
                    className="border border-black text-black h-8 px-4"
                    defaultValue={revPathAddress}
                    {...register("revPathAddress", { required: "Must set." })}
                  />
                  <R3vlProvider client={r3vlClient}>
                    <ReveelForm
                      revPathName={getValues("collectionName")}
                      setRevPathAddress={setRevPathAddress}
                      revPathAddress={revPathAddress}
                    />
                  </R3vlProvider>
                  <p className="text-red-600 text-sm">
                    <ErrorMessage errors={errors} name="revPathAddress" />
                  </p>
                </div>
                ...
                <div className="border border-white rounded-md p-4">
                  ...
                  <div className="flex flex-col gap-3">
                    <p>Description</p>
                    <textarea
                      className="border border-black text-black h-24"
              ...
              <div className="flex flex-col border border-white p-4 rounded-md h-fit w-[500px]">
                ...
                <div className="pt-3 flex flex-col gap-3">
                  <div className="pb-2 flex gap-1">
                    ...
                    <InfoField
                      ...
                      yDirection={"bottom"}
                      infoText={
                        "Please enter a percentage that you would like to receive from the value of every sale.  We use EIP 2981 (NFT Royalty Standard)."
                      }
                    />
                  </div>
                ...
                <div className="flex flex-col gap-3">
                  ...
                  />
                  <p className="text-red-600 text-sm">
                    <ErrorMessage errors={errors} name="saleStartDate" />
                  </p>
                </div>
                ...
                <div className="flex flex-col gap-3">
                  ...
                  />
                  <p className="text-red-600 text-sm">
                    <ErrorMessage errors={errors} name="saleEndDate" />
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col justify-center items-center flex-wrap items-center gap-4">
              ...
              >
                <input
                  ...
                  type="submit"
                  className="cursor-pointer bg-white text-violet-500 hover:opacity-80 font-medium px-8 py-1 rounded-full"
                  value={isDeploying ? "pending..." : "Create Project"}
                />
              </button>
              <p className="pt-4">
                {showLink ? (
                  <a
                    target="_blank"
                    className="tracking-widest font-medium border-b hover:border-b-2 border-violet-500 text-violet-500 hover:border-violet-200 hover:text-violet-200"
                    href={`https://hq.decent.xyz/5/Editions/${link}`}
                    rel="noreferrer"
                  >
                    View on Decent
                  </a>
                  ...
                  <span className="italic text-sm">
                    be patient, wallet confirmation can take a sec
                  </span>
                )}
              </p>
            </div>
